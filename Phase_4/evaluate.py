"""
Phase 4: Temporal Evaluation Script
Evaluates base vs fine-tuned Qwen2-VL-2B on OpenPack clips.
Computes OCA, tIoU@0.5, AA@1 and generates comparison charts.

Usage:
    python evaluate.py \
        --dataset_path /kaggle/input/datasets/.../training_data_samples \
        --frames_path   /kaggle/input/datasets/.../S0500 \
        --finetuned_checkpoint ./checkpoints/final \
...
PROCEDURAL_NEXT = {          # dominant → most likely next op (prior knowledge)
...
def load_finetuned_model(checkpoint_path, processor):
...
# INFERENCE
# ──────────────────────────────────────────────────────────────────────────────
def run_inference(model, processor, pair: dict) -> dict:
    """Run model on one sample pair, return parsed JSON dict."""
...
def evaluate_model(model, processor, pairs: list) -> dict:
    """Run full evaluation loop, return per-clip results + aggregate metrics."""
    results = []

    for i, pair in enumerate(pairs):
        print(f"  [{i+1:02d}/{len(pairs)}] {pair['clip_id']} …", end=" ", flush=True)

        pred   = run_inference(model, processor, pair)
        ...
        print(f"{status}  OCA={oca_ok}  tIoU={tiou:.2f}  AA={aa_ok}")
    ...
    return {
        "aggregate": {"OCA": round(float(oca), 4),
...
def plot_comparison(base_res: dict, ft_res: dict, out_dir: Path):
    ...
    for ax, metric, desc, bv, fv in zip(axes, metrics, descriptions, b_vals, f_vals):
        ...
        ax.set_ylabel("Score")
        ax.axhline(1/9, color=COLORS["bad"], linestyle="--",
                   linewidth=1.2, label="Random (1/9)")
        ax.axhline(0.5, color=COLORS["good"], linestyle=":",
                   linewidth=1.2, label="Target (0.5)")
    ...
    # ── 3. Per-op accuracy heatmap ──────────────────────────────────────────
    ops = sorted(set([r["gt_op"] for r in base_res["per_clip"]
                                  + ft_res["per_clip"]]))
    ...
    ax.bar(x - width/2, [b_acc[o]  for o in ops], width,
           color=COLORS["base"],      label="Base",       edgecolor="white")
    ax.bar(x + width/2, [ft_acc[o] for o in ops], width,
    ...
    # ── 4. Radar chart — overall profile ───────────────────────────────────
    labels  = ["OCA", "tIoU@0.5", "AA@1"]
    n       = len(labels)
    angles  = np.linspace(0, 2*np.pi, n, endpoint=False).tolist()
    angles += angles[:1]

    b_vals_r  = [base_res["aggregate"][m] for m in labels] + \
                [base_res["aggregate"][labels[0]]]
    ft_vals_r = [ft_res["aggregate"][m]   for m in labels] + \
                [ft_res["aggregate"][labels[0]]]
    ...
    names  = ["OCA", "tIoU@0.5", "AA@1"]
    b_sc   = [base_res["aggregate"][m] for m in names]
    ft_sc  = [ft_res["aggregate"][m]   for m in names]
    xp     = np.arange(len(names))
    ax1.bar(xp - 0.2, b_sc,  0.38, color=COLORS["base"],      label="Base")
    ax1.bar(xp + 0.2, ft_sc, 0.38, color=COLORS["finetuned"], label="Fine-tuned")
    ...
    for xi, (bv, fv) in enumerate(zip(b_sc, ft_sc)):
    ...
    # — Delta bar (top-center) —
    ax2 = fig.add_subplot(gs[0, 1])
    deltas = [ft_res["aggregate"][m] - base_res["aggregate"][m] for m in names]
    bar_colors = [COLORS["good"] if d >= 0 else COLORS["bad"] for d in deltas]
    ax2.bar(names, deltas, color=bar_colors, edgecolor="white")
    ...
    # — tIoU per clip (bottom, full width) —
...
# MAIN
# ──────────────────────────────────────────────────────────────────────────────
def load_pairs(dataset_path: Path, frames_path: Path, max_clips: int = 30) -> list:
    pairs = []
...
def main():
    parser = argparse.ArgumentParser(description="Phase 4: Evaluate Base vs Fine-Tuned VLM")
    ...
    pairs = load_pairs(Path(args.dataset_path), Path(args.frames_path), args.max_clips)
    ...
    ft_model = load_finetuned_model(args.finetuned_checkpoint, processor)
    ...
    print(f"\n✅ results.json saved to {out_dir/'results.json'}")

    # ── Plots ─────────────────────────────────────────────────────────────
    print("\n═══ Generating visualisations ═══")
    plot_comparison(base_results, ft_results, out_dir)
